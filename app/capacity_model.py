...
def calculate_team_capacity(team_df, sprint_length_weeks, vacations_df=None, sprints_df=None):
    """
    Calculates capacity per person and aggregates total capacity per competency per sprint.
    Now supports deductions from vacations based on calendar dates.
    Returns: Dict mapping sprint_num -> {competency -> capacity}
    """
    if team_df.empty:
        return {}
    
    # Base capacity per person
    ...
    if sprints_df is not None and not sprints_df.empty:
        ...
        # Ensure we cover at least 50 if the calendar is short
    ...
    for s_num in sprint_nums:
                ...
                try:
                    ...
                    for _, v_row in vacations_df.iterrows():
                        ...
                        if overlap_start <= overlap_end:
                            # It's an inclusive range of days off
                            ...
                            if not temp_team[person_mask].empty:
                                # Calculate deduction proportionally based on the original full sprint capacity
                    ...
                    # If date parsing fails, ignore vacation for this row
                    pass
        ...
        # Ensure all competencies are present
